...
int main() {
  // your code goes here
    int total_coins, sum;
    cout << "Masukkan banyak coin\t\t: ";
    cin >> total_coins;
    cout << "Coin apa aja\t\t\t: ";
    for(int i=0;i<total_coins;i++){
        cin >> coins[i]; 
    }
    cout << "Sum yang diinginkan\t\t: ";
    cin >> sum;

    int ans = minCoins(sum, total_coins);
    if(ans == INT_MAX || ans == -2147483647) cout << "Gabisa NJERRR!!";
    else{
    cout <<"Jumlah minimal coin adalah\t: "<< ans;    
    }
}